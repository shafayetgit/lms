...
import { useCreateEmailTemplateMutation } from "@/features/shared/emailTemplateAPI"
...
export default function EmailTemplateCreateDialog({ open, handleCDialogClose }) {
  ...
  async function handleSubmit(e) {
    ...
    const errs = {}
    if (!values.name?.trim()) errs.name = "Template Name is required"
    ...
    try {
      ...
      toast.error(err?.data?.message || err?.data?.detail || "Failed to create email template")
    }
  }
    ...
    >
      <Box sx={{ mt: 1 }}>
        <CForm
          ...
          btnProps={{ loading: isLoading, label: "Create" }}
          width="100%"
        >
          <Stack spacing={3}>
