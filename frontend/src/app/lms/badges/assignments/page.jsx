...
function AssignDialog() {
  ...
  const handleAssign = async () => {
    ...
    try {
      await assign({
        badge_public_id: selectedBadge.public_id,
        member_public_id: selectedStudent.public_id,
      }).unwrap()
  ...
  return (
    ...
    >
      <CForm
        width="40rem"
        btnProps={{ loading: isAssigning, label: "Assign" }}
        dialog
        onSubmit={e => {
          e.preventDefault()
          handleAssign()
...
function AssignmentList({ action }) {
  ...
  return (
    <CDataTable
      ...
      loading={isLoading}
      action={action}
      bulkDelete={{ model: "BadgeAssignment", invalidateTag: "BADGES" }}
    />
  )
}
