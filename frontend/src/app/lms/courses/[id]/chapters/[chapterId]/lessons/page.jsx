...
function LessonList({ chapterId, courseId, sortedLessons, selectedIds, setSelectedIds }) {
  ...
  const lessonIds = React.useMemo(() => sortedLessons.map(l => l.id), [sortedLessons])
...
export default function Page() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useReadLessonsByChapterQuery({ chapterId })
  const theme = useTheme()
  ...
  return (
    <PermissionGuard resource="lesson" action="read">
      <CModuleLayout navigators={navigators} helpTips={LESSON_TIPS.list}>
        <Box
          sx={{
            display: "flex",
            alignItems: "center",
