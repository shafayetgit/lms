...
import { useReadInstructorQuery, useUpdateInstructorMutation } from "@/features/instructor/instructorAPI";
import { useAttachMutation } from "@/features/media/mediaApi";
import { instructorUpdateSchema } from "@/schema/instructor";
...
export default function Page() {
  ...
  const { data = {}, isLoading } = useReadInstructorQuery(
    { id },
    { refetchOnMountOrArgChange: true, skip: !id }
  );

  const [update, { isLoading: isUpdating }] = useUpdateInstructorMutation();
  ...
  const formik = useFormik({
    ...
    validationSchema: instructorUpdateSchema,
    ...
    onSubmit: async (values, { resetForm, setErrors }) => {
      ...
      try {
        ...
        const instructorResponse = await update({ id, body: updatePayload }).unwrap();
        ...
        if (avatar) {
          ...
          try {
            ...
            // Step 3: Attach media to instructor
            ...
            toast.warning(
              "Instructor updated successfully, but media attachment failed. You can retry uploading the avatar."
            );
          } finally {
            setUploadProgress(0);
          }
        }
        toast.success(instructorResponse?.message || "Instructor updated successfully");
