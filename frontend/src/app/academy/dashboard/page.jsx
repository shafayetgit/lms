...
import { SchoolOutlined, MilitaryTechOutlined, ChevronRightOutlined, CheckCircleOutlined } from "@mui/icons-material";
...
const DASHBOARD_HELP_TIPS = {
  description: "Welcome to your Academy Dashboard! Here you can track your active learning progress, streaks, view upcoming sessions, and manage your achievements.",
  tips: [
    {
...
export default function AcademyDashboard() {
  ...
  const { data: enrollmentsData, isLoading: enrollmentsLoading, isError: enrollmentsError } = useReadMyEnrollmentsQuery(
    { size: 100 },
    { refetchOnMountOrArgChange: true }
  );
  const { data: badgesData, isLoading: badgesLoading, isError: badgesError } = useReadBadgeAssignmentsQuery({ size: 3 });
  const { data: summaryRes, isLoading: summaryLoading, isError: summaryError } = useReadStudentDashboardSummaryQuery();
  ...
  const activeEnrollment = enrollments.find(e => e.progress > 0 && e.progress < 100) || enrollments.find(e => Math.round(e.progress || 0) === 0);
    ...
    <CModuleLayout helpTips={DASHBOARD_HELP_TIPS}>
        ...
        {activeEnrollment && (
          <Grid size={{ xs: 4, sm: 8, md: 7 }}>
            ...
            >
              ...
              <Box>
                <Typography variant="caption" sx={{ textTransform: "uppercase", fontWeight: 700, color: "primary.main", letterSpacing: "0.05em", display: "block", mb: 1 }}>
            ...
            >
              <Stack direction="row" justifyContent="space-between" alignItems="center">
                ...
                <Box sx={{ textAlign: "right" }}>
                  <Typography variant="h4" sx={{ fontWeight: 900, color: "warning.main", lineHeight: 1 }}>
                  ...
                  {last7Days.map((day) => (
                    <Box
                      ...
                      sx={{
                        ...
                        py: 1,
                        borderRadius: 1,
                        bgcolor: day.isToday ? (theme) => alpha(theme.palette.primary.main, 0.08) : "transparent",
                        border: day.isToday ? "1px solid" : "1px solid transparent",
                        borderColor: day.isToday ? "primary.main" : "transparent",
                      }}
                    >
                      <Typography variant="caption" sx={{ fontWeight: 700, color: day.isToday ? "primary.main" : "text.secondary", fontSize: "0.75rem", mb: 0.5 }}>
                        {day.name}
                      </Typography>
                      <Avatar
                        sx={{
                          ...
                          fontSize: "0.85rem",
                          fontWeight: 700,
                          boxShadow: day.isActive ? (theme) => `0 2px 8px ${alpha(theme.palette.warning.main, 0.4)}` : "none",
                        }}
                      >
      ...
      <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }} sx={{ mb: 5 }}>
        <Grid size={{ xs: 4, sm: 4, md: 4 }}>
          ...
          >
            <Avatar sx={{ width: 56, height: 56, bgcolor: theme => alpha(theme.palette.primary.main, 0.15), borderRadius: 1 }}>
        ...
        <Grid size={{ xs: 4, sm: 4, md: 4 }}>
          ...
          >
            <Avatar sx={{ width: 56, height: 56, bgcolor: theme => alpha(theme.palette.success.main, 0.15), borderRadius: 1 }}>
        ...
        <Grid size={{ xs: 4, sm: 4, md: 4 }}>
          ...
          >
            <Avatar sx={{ width: 56, height: 56, bgcolor: theme => alpha(theme.palette.secondary.main, 0.15), borderRadius: 1 }}>
      ...
      {(upcomingClasses.length > 0 || upcomingEvaluations.length > 0) && (
        <Grid container spacing={{ xs: 3, md: 4 }} columns={{ xs: 4, sm: 8, md: 12 }} sx={{ mb: 5 }}>
                  ...
                  >
                    ...
                    <Stack spacing={1} sx={{ color: "text.secondary", fontSize: "0.875rem" }}>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        <Typography variant="body2">📅 {dayjs(cls.date).format("DD MMMM YYYY")}</Typography>
                      </Box>
                      <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        <Typography variant="body2">⏰ {cls.time} ({cls.duration} mins)</Typography>
                      </Box>
                      {cls.host && (
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                {upcomingEvaluations.map((evalSession) => (
                  ...
                  >
                    <Box>
                      <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={1}>
                        <Typography variant="subtitle1" sx={{ fontWeight: 700, color: "text.primary" }}>
                          {evalSession.course?.title || evalSession.batch?.title || "Evaluation Session"}
                        </Typography>
                        <Paper
                          elevation={0}
                          sx={{
                    ...
                    <Stack spacing={1} sx={{ color: "text.secondary", fontSize: "0.875rem" }}>
                      {evalSession.date && (
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                          <Typography variant="body2">📅 {dayjs(evalSession.date).format("DD MMMM YYYY")}</Typography>
                        </Box>
                      )}
                      {evalSession.start_time && (
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                          <Typography variant="body2">⏰ {evalSession.start_time} - {evalSession.end_time}</Typography>
                        </Box>
                      )}
                      {evalSession.evaluator && (
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                          <Typography variant="body2">🎓 Evaluator: {evalSession.evaluator.full_name}</Typography>
                        </Box>
                      )}
                    </Stack>
                  </Card>
        ...
        <Grid size={{ xs: 4, sm: 8, md: 7 }}>
          ...
          {enrollments.length > 0 ? (
            ...
            >
              ...
              <Button component={Link} href="/courses" variant="contained" size="small" sx={{ borderRadius: 1 }}>
                Explore Courses
              </Button>
            </Paper>
          )}
                ...
                return (
                  ...
                  >
                    <Avatar
                      ...
                      sx={{ width: 48, height: 48, bgcolor: theme => alpha(theme.palette.secondary.main, 0.15) }}
                    >
