...
import { useGetCheckoutLinkMutation } from "@/features/payment/paymentApi";
...
export default function EnrollButton({
  ...
  async function initiateCheckout(e) {
    ...
    try {
      ...
      const detail = err?.data?.message || err?.data?.detail || err?.message || "Enrollment failed. Please try again.";
      toast.error(detail);
    }
  }
  ...
  return (
    <>
      <CButton
        ...
        onClick={handleClick}
        icon={isLoading ? <CircularProgress size={16} color="inherit" /> : (paidItem ? <CreditCard /> : <School />)}
      />
