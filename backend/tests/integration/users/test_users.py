...
@pytest.mark.asyncio
async def test_login_and_me(unauthenticated_client: AsyncClient, db_session: AsyncSession):
    ...
    response = await unauthenticated_client.post("/api/v1/auth/token", json=login_payload)
    assert response.status_code == 200
    ...
    response_me = await unauthenticated_client.get("/api/v1/users/me", headers=headers)
    print("DEBUG RESPONSE ME:", response_me.json())
...
async def test_update_user(client: AsyncClient, db_session: AsyncSession):
    ...
    try:
        ...
        assert response.status_code == 200
        assert response.json()["data"]["first_name"] == "New"
    finally:
