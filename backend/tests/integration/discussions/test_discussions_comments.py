...
async def test_category(db_session: AsyncSession):
    category = Category(name=f"Test Cat {uuid.uuid4().hex}", slug=f"test-cat-{uuid.uuid4().hex}")
...
async def test_course(db_session: AsyncSession, test_admin, test_category):
    ...
    course = Course(
        title="Test Course",
        slug=f"test-course-{uuid.uuid4().hex}",
...
@pytest_asyncio.fixture
async def test_chapter(db_session: AsyncSession, test_course):
    chapter = Chapter(title="Test Module", course_id=test_course.id, order_index=1)
...
@pytest_asyncio.fixture
async def test_lesson(db_session: AsyncSession, test_chapter):
    lesson = Lesson(
        title="Test Lesson",
        slug=f"test-lesson-{uuid.uuid4().hex}",
...
async def test_create_discussion(client: AsyncClient, test_admin, test_course, test_lesson):
    ...
    payload = {
        ...
        "lesson_public_id": test_lesson.public_id,
...
async def test_create_comment(client: AsyncClient, test_admin, test_course):
    ...
    disc_payload = {"course_public_id": test_course.public_id, "title": "Discussion 1"}
...
async def test_nested_comments(client: AsyncClient, test_admin, test_course):
    ...
    assert r_res.json()["parent_id"] == parent_id
...
@pytest.mark.asyncio
async def test_security_access_allowed_for_student(client: AsyncClient, test_student, test_course):
    """Test that a student can access these endpoints."""
    ...
    payload = {"course_public_id": test_course.public_id, "title": "Student Discussion"}
    response = await client.post("/api/v1/discussions/", json=payload)
    assert response.status_code == 201
