...
class Settings(BaseSettings):
    ...
    OTP_RESEND_MAX_ATTEMPTS: int = Field(
        default=5, validation_alias="OTP_RESEND_MAX_ATTEMPTS"
    )
    OTP_RESEND_COOLDOWN_SECONDS: int = Field(
        default=60, validation_alias="OTP_RESEND_COOLDOWN_SECONDS"
    )

    # SMS Configuration (Twilio)
    SMS_PROVIDER: str = Field(
        default="twilio", validation_alias="SMS_PROVIDER"
    )  # 'twilio' or 'vonage'
    ...
    TOTP_WINDOW: int = Field(
        default=1, validation_alias="TOTP_WINDOW"
    )  # Allow 1 time window drift

    # Login Settings
    MAX_LOGIN_ATTEMPTS: int = Field(default=5, validation_alias="MAX_LOGIN_ATTEMPTS")
    LOGIN_ATTEMPT_LOCKOUT_MINUTES: int = Field(
        default=15, validation_alias="LOGIN_ATTEMPT_LOCKOUT_MINUTES"
    )
    ...
    PASSWORD_REQUIRE_DIGITS: bool = Field(
        default=True, validation_alias="PASSWORD_REQUIRE_DIGITS"
    )
    ...
    MEDIA_PROVIDER: str = Field(
        default="cloudinary", validation_alias="MEDIA_PROVIDER"
    )  # 'cloudinary', 's3', 'imagekit', 'local'
    ...
    if settings is None:
        ...
        try:
            ...
            settings.ACCESS_TOKEN_EXPIRE_MINUTES = (
                project_cfg.security.session.access_token_expire_minutes
            )
            settings.REFRESH_TOKEN_EXPIRE_DAYS = (
                project_cfg.security.session.refresh_token_expire_days
            )
            ...
            settings.LOGIN_ATTEMPT_LOCKOUT_MINUTES = (
                project_cfg.security.login.lockout_minutes
            )
            settings.PASSWORD_MIN_LENGTH = project_cfg.security.password.min_length
            settings.PASSWORD_REQUIRE_UPPERCASE = (
                project_cfg.security.password.require_uppercase
            )
            settings.PASSWORD_REQUIRE_LOWERCASE = (
                project_cfg.security.password.require_lowercase
            )
            settings.PASSWORD_REQUIRE_DIGITS = (
                project_cfg.security.password.require_digits
            )
            settings.PASSWORD_REQUIRE_SPECIAL_CHARS = (
                project_cfg.security.password.require_special_chars
            )
