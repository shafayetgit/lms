...
Handles registration, login, 2FA, OAuth2, password management, etc.
"""
...
async def register(
    ...
    User account is created but email verification is required before login.
    """
    try:
...
async def verify_totp(
    ...
    raise HTTPException(
        ...
        detail="TOTP verification needs login context",
    )
...
async def github_login(
    ...
    raise HTTPException(
        status_code=status.HTTP_501_NOT_IMPLEMENTED,
        detail="GitHub OAuth integration coming soon",
    )
